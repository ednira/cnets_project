...
def visualization(graph, act_total, activities, dep_dict, cnet_inbindings, cnet_outbindings):
    ...
    graph.attr(nodesep='0.3', ranksep='.3', pad='1')
    ...
    for index, row in vis_edges.iterrows():
        ...
        if row['type'] == 'vis_binding':
            ...
            graph.edge(row['source'], row['target'], label=str(row['label']), color=row['color'], penwidth=str(row['width']), minlen=length_str, arrowhead='vee', dir='forward', splines='spline')
            ...
            graph.edge(row['source'], row['target'], label=str(row['label']), color=row['color'], penwidth=str(row['width']), minlen=length_str, arrowhead='none', labeldistance='10', splines='spline')
