...
def visualization(graph, act_total, activities, dep_dict, cnet_inbindings, cnet_outbindings):
    ...
    for index, row in nodes_df.iterrows():
        ...
        if row['type'] != 'activity':
            ...
            graph.node(row['node'], label=label, color=row['color'], shape=shape, width=width, height=height, style='filled')
    ...
    for index, row in vis_edges.iterrows():
        ...
        if row['type'] == 'vis_binding':
            graph.edge(row['source'], row['target'], label=str(row['label']), color=row['color'], style='dashed', penwidth=str(row['width']), arrowhead='none', splines='spline')
        elif row['arrow']:
            # Add arrowhead to the edge that points to the target node
            graph.edge(row['source'], row['target'], label=str(row['label']), color=row['color'], penwidth=str(row['width']), minlen=length_str, arrowhead='normal', dir='forward')
        elif row['type'] == 'ldd_visualization':
            graph.edge(row['source'], row['target'], color=row['color'], style='dotted', penwidth=str(row['width']), arrowhead=row['arrow'], splines='spline')
        else:
